...
public class DefaultJSONArray implements JSONArray {
    ...
    public DefaultJSONArray(ObjectToStringChain... chains) {
        this();
        this.chain = SerializeChainFactory.getChain(chains);
    }


    public DefaultJSONArray(int size) {
        this.list = new ArrayList<>(size);
    }

    public DefaultJSONArray(int size, ObjectToStringChain... chains) {
        this.list = new ArrayList<>(size);
        this.chain = SerializeChainFactory.getChain(chains);
    }

    public DefaultJSONArray(List list) {
        if (list != null) {
            this.list = list;
        } else {
    ...
    public DefaultJSONArray(List list, ObjectToStringChain... chains) {
        this(list);
        this.chain = SerializeChainFactory.getChain(chains);
    }
    ...
    public static DefaultJSONArray parse(String json) {
        Collection list = JSONDeserializer.parseToJSONArray(json.trim());
        if (list.isEmpty()) {
    ...
    public static <T> List<T> parse(String json, Class<T> clazz) {
        DefaultJSONArray array = DefaultJSONArray.parse(json);
    ...
    public DefaultJSONObject getJSONObject(int index) {
        return getObject(index, DefaultJSONObject.class);
    }

    public DefaultJSONArray getJSONArray(int index) {
        return getObject(index, DefaultJSONArray.class);
    }
